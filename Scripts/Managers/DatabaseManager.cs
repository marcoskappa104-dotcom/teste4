...
namespace RPG.Managers
{
    ...
    /// <summary>
    /// Resultado de tentativa de login com delay sugerido para mitigação de timing-attack.
    /// </summary>
    ...
    ///
    ///   1. MAX_QUEUE_BURST AGORA FAZ ALGO REAL:
    ///      Antes, o comentário dizia "cede momentaneamente para evitar
    ///      monopolizar CPU" mas o código não cedia nada — só resetava
    ///      um contador. Agora, ao atingir BURST, chamamos Thread.Yield()
    ///      de verdade, permitindo que outros threads do servidor (rede,
    ///      etc) progridam em picos de save.
    ///
    ///   2. SnapShotCharacterRow CENTRALIZADO (mantido):
    ///      Adicionar campos no schema requer só atualizar este método.
    ///
    ///   3. CLEANUP GRACIOSO NO SHUTDOWN (mantido):
    ///      FlushAndClose processa fila restante antes de fechar conexão.
    /// </summary>
    public class DatabaseManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// A cada N writes processadas em burst, o thread cede a CPU via
        /// Thread.Yield() para que outros threads do servidor (rede, etc)
        /// possam progredir. Sem isso, num pico de 100+ saves simultâneos
        /// o thread de DB monopolizaria um core.
        /// </summary>
        ...
        /// <summary>
        /// Processa todas as escritas pendentes. A cada MAX_QUEUE_BURST,
        /// cede a CPU via Thread.Yield() para evitar monopolizar um core
        /// em picos de save (ex: 50 players salvando após batalha).
        /// </summary>
        private void DrainQueue()
        {
            ...
            while (_writeQueue.TryDequeue(out Action action))
            {
                ...
                {
                    processed = 0;
                    // Cede a CPU para outros threads (rede, main thread, etc).
                    // Não dorme — só sinaliza ao scheduler que pode mudar de
                    // thread se houver outro pronto.
                    Thread.Yield();
                }
            }
        }
        ...
        /// <summary>
        /// Snapshot completo do CharacterData. Centralizado para evitar drift
        /// quando novos campos são adicionados ao schema.
        /// </summary>
