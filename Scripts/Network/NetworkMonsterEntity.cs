...
namespace RPG.Network
{
    ...
    ///
    /// === MUDANÇAS DESTA VERSÃO (Lote de correções) ===
    ///
    ///   1. PROJÉTIL VOA MESMO EM MISS:
    ///      Antes, se o atacante ranged errasse, MISS aparecia instantaneamente
    ///      no alvo distante — flecha "sumia no ar". Agora o projétil é
    ///      spawnado com flag isMiss=true e mostra "MISS" no impacto. UX
    ///      consistente: o player vê a flecha viajar, feedback chega no impacto.
    ///
    ///   2. GUARD EM LookRotation:
    ///      Se spawnPos == ImpactPoint (point-blank com arma ranged),
    ///      LookRotation(zero) logava warning. Fallback para forward do atacante.
    ///
    ///   3. AGGRO BUFFER CONFIGURÁVEL:
    ///      aggroOverlapBufferSize virou campo serializado. Warning quando satura.
    ///
    ///   4. SHOOTER NETID NO DAMAGE LOG VIA PROJÉTIL:
    ///      ServerTakeProjectileDamage agora recebe shooterNetId e credita o
    ///      dano no log para XP correto mesmo em kills à distância.
    /// </summary>
    ...
    public class NetworkMonsterEntity : NetworkBehaviour, ITargetable
    {
        ...
        [Tooltip("Tamanho do buffer de OverlapSphere para detecção de aggro. " +
                 "Em raids/eventos com muitos players próximos, aumente.")]
        [SerializeField] private int aggroOverlapBufferSize = 16;
        ...
        private void Awake()
        {
            ...
            int bufSize = Mathf.Max(4, aggroOverlapBufferSize);
            _aggroOverlapBuffer = new Collider[bufSize];
        }
        ...
        private void TryAggro()
        {
            ...
            // Warning quando o buffer satura — sinaliza necessidade de aumentar
            if (count >= _aggroOverlapBuffer.Length)
            {
                Debug.LogWarning(
                    $"[NetworkMonsterEntity] '{monsterDisplayName}': buffer de aggro saturado " +
                    $"({_aggroOverlapBuffer.Length}). Considere aumentar aggroOverlapBufferSize " +
                    "no Inspector.");
            }
        ...
        // ══════════════════════════════════════════════════════════════════
        // Recebimento de dano POR PROJÉTIL (hit ou miss)
        // ══════════════════════════════════════════════════════════════════

        /// <summary>
        /// Projétil acertou. Credita o shooter no damage log para XP correto.
        /// </summary>
        [Server]
        public void ServerTakeProjectileDamage(float dmg, bool crit, uint shooterNetId)
        {
            ...
            if (shooterNetId != 0)
            {
                if (!_damageLog.ContainsKey(shooterNetId))
                    _damageLog[shooterNetId] = 0f;
        ...
        /// <summary>
        /// Projétil errou. Mostra "MISS" no impacto. Não tira HP.
        /// </summary>
        [Server]
        public void ServerTakeProjectileMiss()
        {
            if (_isDead || _deathProcessed) return;
            RpcShowMiss(ImpactPoint);
        }
        ...
        // ══════════════════════════════════════════════════════════════════
        // CmdBasicAttack — projétil voa mesmo em miss
        // ══════════════════════════════════════════════════════════════════
        ...
        public void CmdBasicAttack(uint attackerNetId, float clientAttackRange)
        {
            ...
            // === RANGED MISS: projétil voa, miss aplicado no impacto ===
            if (!hit)
            {
                if (profile.UsesProjectile)
                {
                    ApplyAggroReaction(attacker);
                    SpawnAttackProjectile(attacker, profile, 0f, false, attacker.netId, isMiss: true);
                }
                else
                {
            ...
            if (profile.UsesProjectile)
            {
                // Ranged HIT: damage log atualizado em ServerTakeProjectileDamage
                SpawnAttackProjectile(attacker, profile, dmg, crit, attacker.netId, isMiss: false);
            }
            else
            {
                // Melee HIT: damage log e dano instantâneos
        ...
        /// <summary>
        /// Spawna projétil server-side. isMiss=true significa que o projétil
        /// vai voar mas ao impactar mostra MISS em vez de dano.
        /// </summary>
        [Server]
        private void SpawnAttackProjectile(NetworkPlayer attacker, WeaponAttackProfile profile,
                                           float damage, bool crit, uint shooterNetId, bool isMiss)
        {
            var nm = RPGNetworkManager.singleton as RPGNetworkManager;
            var prefab = nm?.GetProjectilePrefab(profile.Type);
            if (prefab == null)
            {
                Debug.LogWarning($"[Combat] Sem prefab de projétil para {profile.Type}. " +
                                 "Aplicando feedback instantâneo como fallback.");
                if (isMiss) RpcShowMiss(ImpactPoint);
                else
                {
            ...
            // Guard: se spawnPos ≈ ImpactPoint (point-blank), LookRotation(zero)
            // logaria warning. Usa forward do atacante como fallback.
            Vector3 lookDir = ImpactPoint - spawnPos;
            Quaternion spawnRot = lookDir.sqrMagnitude > 0.0001f
            ...
            proj.ServerInitialize(this, profile.ProjectileSpeed, damage, crit, shooterNetId, isMiss);
        }
