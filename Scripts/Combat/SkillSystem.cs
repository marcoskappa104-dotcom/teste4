...
namespace RPG.Combat
{
    ...
    ///
    ///   1. CANCEL CAST/WALK: DOCUMENTAÇÃO DE CONTRATO:
    ///      CancelCast pode ser chamado a qualquer momento, inclusive
    ///      enquanto a coroutine CastSequence está no yield. Em paths
    ///      assim, StopCoroutine para a execução imediatamente — o
    ///      `_isCasting = false` dentro da coroutine NÃO roda. Por isso
    ///      setamos `_isCasting = false` EXTERNAMENTE no método cancel.
    ///      Mesmo princípio para CancelPendingWalk.
    ///
    ///   2. STOPPING DISTANCE IDLE VEM DE GameConstants.Movement:
    ///      Consistência com PlayerEntity/NetworkPlayer/Controller.
    ///
    ///   3. SUBSCRIÇÃO EM OnDeathChanged E OnTargetChanged (mantido):
    ///      Cancelamento reativo de walk/cast.
    /// </summary>
    ...
    public class SkillSystem : NetworkBehaviour
    {
        ...
        /// <summary>
        /// Cancela um cast em andamento. PODE ser chamado a qualquer momento,
        /// inclusive enquanto a coroutine CastSequence está aguardando yield.
        /// Por isso o _isCasting é setado AQUI (não dentro da coroutine),
        /// garantindo que o estado fique consistente mesmo se a coroutine
        ...
        ///
        /// Mesmo princípio do CancelCast: seta estado externo, não confia
        /// na coroutine interrompida.
        /// </summary>
        ...
        private IEnumerator WalkThenSendCmd(int index, SkillData skill, ITargetable target)
        {
            ...
            {
                StopAgent();
                if (timeout <= 0f) Log($"Walk: timeout após {WALK_TIMEOUT}s.");
            }
        }
        ...
        /// <summary>
        /// Para o agent suavemente. Stopping distance vem de
        /// GameConstants.Movement para consistência entre sistemas.
        /// </summary>
        private void StopAgent()
        {
            if (_agent == null || !_agent.isOnNavMesh) return;
            _agent.ResetPath();
            _agent.stoppingDistance = GameConstants.Movement.AGENT_IDLE_STOPPING_DISTANCE;
            _lastWalkDestination    = Vector3.positiveInfinity;
        }
