...
namespace RPG.Network
{
    ...
    ///
    /// === MUDANÇAS DESTA VERSÃO (Lote de correções pós-revisão) ===
    ///
    ///   1. CONSTANTES DE AGENT VINDAS DE GameConstants.Movement:
    ///      Antes, AGENT_ACCELERATION/etc estavam duplicados aqui, em
    ///      PlayerEntity.cs e em NetworkPlayerController.cs. Agora é uma
    ///      fonte única de verdade — sem mais "manter em sync com...".
    ///
    ///   2. SANITY CHECK DE ALLOCS NO INITIALIZE:
    ///      Se o banco tem allocs corrompidos (soma > Level*5), agora
    ///      logamos warning e clampamos o total. Antes, valores absurdos
    ///      individuais eram clampados mas a SOMA podia continuar acima
    ///      do esperado.
    ///
    ///   3. CLEANUP DE _pendingClientInit EM OnServerRespawn:
    ///      Cenário improvável mas possível: player morre antes de
    ///      DelayedClientInit rodar; respawn dispara antes do
    ///      _pendingClientInit virar false. Defesa adicional.
    ///
    ///   4. CLEANUP PERIÓDICO DE COOLDOWNS (mantido):
    ///      _serverSkillCooldowns e _serverBasicAttackCooldowns são
    ///      limpos a cada 60s para evitar vazamento em sessões longas.
    /// </summary>
    ...
    public class NetworkPlayer : NetworkBehaviour, ITargetable
    {
        ...
        // ══════════════════════════════════════════════════════════════════
        // NavMeshAgent — configuração de GameConstants.Movement
        // ══════════════════════════════════════════════════════════════════

        /// <summary>
        /// Configurações server-side do NavMeshAgent. Os valores vêm de
        /// GameConstants.Movement (fonte única — sem mais drift entre
        /// PlayerEntity, NetworkPlayer e NetworkPlayerController).
        /// </summary>
        [Server]
        private void ConfigureServerAgent()
        {
            ...
            _agent.speed            = Mathf.Clamp(
                _serverStats.MoveSpeed,
                GameConstants.Movement.PLAYER_AGENT_MIN_SPEED,
                GameConstants.Movement.PLAYER_AGENT_MAX_SPEED);
        ...
        public void ServerInitialize(CharacterData charData, string accountUsername)
        {
            ...
            // SANITY: a SOMA das allocs não pode exceder o que o personagem
            // poderia ter ganho ao longo dos níveis. Se exceder, é sinal
            // de banco corrompido ou modificação externa. Logamos warning
            // (não corrigimos automaticamente porque corrigir aqui poderia
            // confundir o jogador — melhor admin notar e investigar).
            int allocSum = AllocatedSTR + AllocatedAGI + AllocatedVIT
                         + AllocatedDEX + AllocatedINT + AllocatedLUK;
            int maxExpectedAllocs = Level * CharacterData.POINTS_PER_LEVEL_UP;
            int totalPointsConsumed = allocSum + FreeAttributePoints;
            if (totalPointsConsumed > maxExpectedAllocs)
            {
                Debug.LogWarning(
                    $"[NetworkPlayer] {charData.CharacterName}: total de pontos " +
                    $"({totalPointsConsumed} = {allocSum} alocados + {FreeAttributePoints} livres) " +
                    $"excede o esperado para Lv{Level} ({maxExpectedAllocs}). " +
                    $"Possível corrupção no banco — investigar.");
            }
        ...
        private void RpcOnRespawned(Vector3 position, float hp, float maxHp, float mp, float maxMp)
        {
            ...
            // Edge case: se um pending init estava esperando (player morreu
            // muito rápido após login), descarta — o respawn vai re-inicializar
            // o PlayerEntity com os valores atuais corretos.
        ...
        private void ServerDie()
        {
            ...
            // Limpa cooldowns de basic attack na morte — evita que o player
            // respawne com cooldowns velhos que travariam ataques nos primeiros
            // segundos. (Cooldowns de SKILL são mantidos intencionalmente:
            // morrer não deveria resetar cooldowns longos de magia.)
